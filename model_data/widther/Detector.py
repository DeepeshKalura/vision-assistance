...
class Detector:
    ...
    def process_frames(self):
        for frame in self.receive_frames():
            ...
            if key == ord("q"):
                break
            elif key == ord("a"):  # Capture JPEG image and print width if person detected
                cv2.imwrite("captured_image.jpg", frame)
                if "person" in self.classesList:
                    person_idx = self.classesList.index("person")
                    if person_idx in classLabelIDs:
                        person_bboxs = [bboxs[i] for i in range(len(bboxs)) if classLabelIDs[i] == person_idx]
                        for bbox in person_bboxs:
                            x, y, w, h = bbox
                            print("Width of the person:", w)
